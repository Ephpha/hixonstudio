...
export default function ContactClient() {
  ...
  // Under reduced motion the typewriter never runs, so show the finished line.
  ...
  useEffect(() => {
    if (prefersReducedMotion()) {
      // The subhead is rendered from `subhead` below, which already resolves to
      // the full string under reduced motion — so just reveal the layout.
      const fields = Array.from(
        formCardRef.current?.querySelectorAll(".field") ?? []
      );
      gsap.set([heroRef.current, formCardRef.current, ...fields], {
        opacity: 1,
        y: 0,
      });
      return;
